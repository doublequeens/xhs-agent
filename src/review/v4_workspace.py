...
from src.schemas.v4.review import ReviewWorkspaceManifestV4
...
def _read_revision(paths: ArtifactPaths, relative: str, digest: str) -> bytes:
    ...
    try:
        return read_verified_artifact(paths.revision_root / relative, digest, containment_root=paths.revision_root)
...
def _html_page(inputs: ReviewWorkspaceInputsV4, manifest: ReviewWorkspaceManifestV4 | None, page_paths: Mapping[str, str]) -> bytes:
    ...
    assets = "".join(
        ...
        " human=" + html.escape(item.human_decision) + " sha256=" + html.escape(item.sha256) + "</li>"
...
def _previous_files(inputs: ReviewWorkspaceInputsV4) -> tuple[Path, ReviewWorkspaceManifestV4] | None:
    ...
    for relative, digest in manifest.files.items():
        try:
            read_verified_artifact(root / relative, digest, containment_root=root)
        except ArtifactBindingError as error:
    ...
    try:
        with _lease_context(_open_absolute_directory(paths.revision_root, create=False)) as lease:
            ...
            if previous is not None:
                ...
                previous_contact = read_verified_artifact(
                ...
                for destination, digest in previous_manifest.page_sha256.items():
                    prior = read_verified_artifact(previous_root / destination, digest, containment_root=previous_root)
            ...
            files["decision.json"] = _sha(intake)
...
def verify_review_workspace(workspace: ReviewWorkspaceV4) -> None:
    ...
    try:
        ...
        if not stat.S_ISREG(info.st_mode) or info.st_nlink != 1:
            raise ReviewBindingError("workspace manifest is not a unique regular file")
        raw = read_verified_artifact(manifest_path, _sha(manifest_path.read_bytes()), containment_root=workspace.root)
    ...
    for relative, digest in actual.files.items():
        ...
        try:
            info = path.lstat()
            if not stat.S_ISREG(info.st_mode) or info.st_nlink != 1:
                raise ReviewBindingError("workspace file is linked or not regular")
            read_verified_artifact(path, digest, containment_root=workspace.root)
